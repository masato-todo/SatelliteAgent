...
- **7.2 lfm2_multiturn の真の SSE streaming 化**
  - 現状 `/api/run_agent?provider=lfm25_vl_sft_grpo` は `_run_lfm2_as_events` で
    `agent.lfm2_agent.run_lfm2_agent()` を完走させてから tool_call_log を順に
    SSE event として吐いている (= "after-the-fact replay")。UI 体感は 30-60s の無
    反応 → 一気に全 event 表示
  - `agent/lfm2_agent.py:run_lfm2_agent` を generator に refactor し、各
    tool_call が決まった瞬間に yield するようにすれば、openai_compat / gemini と
    同じ逐次 stream 体感に揃う
  - server 側は `_run_lfm2_as_events` の中身を generator パススルーに置き換え
    るだけ
