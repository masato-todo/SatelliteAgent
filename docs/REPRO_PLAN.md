...
| 8085 | `services/wildfire/Dockerfile` (`lfm25-vl-trl-ft:latest` ベース) | wildfire LoRA (LFM2.5-VL-450M-wildfire) |
...
EXPOSE 8086
```
build:  `docker build -t satelliteagent/lfm2-agent:cu130 -f services/agent/Dockerfile .`
run: `scripts/serve_vllm_lfm2.sh` (既存、`MODEL_PATH` だけ env で指定)

### 3.2 wildfire LoRA (port 8085) — 既存 `services/wildfire/` (transformers+peft)
- `services/wildfire/Dockerfile` は OK (`lfm25-vl-trl-ft:latest` ベース、peft が adapter を runtime 適用)
- `services/wildfire/server.py` / `requirements.txt` はそのまま
...
| `scripts/run_lfm_wildfire.sh` | port 8085 のコンテナ起動 (既存 `services/wildfire/docker-compose.yaml` 経由) |
...
4. Phase 3: `services/agent/Dockerfile` 抽出 + `docker-compose.yaml` 追加
...
- **7.1 wildfire LoRA を VL 部分込みで正しく re-merge** → vLLM merged ckpt 化を再挑戦
  ...
  - 成功すれば services/wildfire 廃止 → vLLM 1 image 2 process (1 docker-compose service にまとめ可)
- ~~**7.2 lfm2_multiturn の真の SSE streaming 化**~~ → **完了 (2026-05-06)**
  - `agent/lfm2_agent.py` に `iter_lfm2_agent` (generator) を追加、`run_lfm2_agent`
    は drain wrapper として後方互換のため残置
  - `app/server.py:_run_lfm2_as_events` は `yield from iter_lfm2_agent(...)` の
    パススルーに簡略化
  - 計測: action は ~0.2s、observation は tool 実行直後 (~6s 後の SimSat fetch
    も適切なタイミングで) 着弾、openai_compat / gemini と同じ逐次 stream 体感
- **7.3 vLLM 上流に LFM2-VL の `multi_modal_projector` LoRA サポート PR**
